...
#'
#' @export
cli_progress <- function(format = "{cli::pb_spin} {cli::pb_bar} {cli::pb_percent} {cli::pb_status}", show_after = 0.0, intrusiveness = getOption("progressr.intrusiveness.terminal", 1), target = "terminal", ...) {
  ...
  reporter <- local({
    ...
    make_pb <- function(format, total, clear, show_after, ...) {
      ...
      args <- c(list(total = total, format = format, clear = clear,
                     show_after = show_after, ...), backend_args)
      pb <<- do.call(progress_bar, args = args)
      pb
    }
